...
We selected this paper because approximate Nash equilibria have both fundamental theoretical importance and practical relevance when exact equilibrium computation or complete payoff information is infeasible. Its appearance in the lecture notes first drew our attention. Its relatively concise presentation made the central argument accessible, while the derivations left implicit offered an opportunity for careful reconstruction and verification. Finally, among the papers satisfying these criteria, our choice was partly random rather than based on a claim that this paper is uniquely representative of the area.

\section{Preliminaries}

For a bimatrix game whose payoffs are normalized to $[0,1]$, we use the standard definitions of $\varepsilon$-NE and $\varepsilon$-WSNE \cite[Section~2]{12WSNE}:

\begin{itemize}
  \item \textbf{$\varepsilon$-NE ($\varepsilon$-Nash Equilibrium)}: Formally, a mixed strategy profile $(\mathbf{x}, \mathbf{y})$ is called an $\varepsilon$-NE if and only if:
        \begin{align*}
...
\begin{theorem}[{Corrected Query Version \cite[Theorem~2]{12WSNE}}]
...
Here $\kappa(\delta) = \left\lceil \dfrac{2 \ln (1/\delta)}{\delta^2} \right\rceil$ is a constant for fixed $\delta \in (0,1)$. A $k$-uniform strategy profile $(\mathbf{w}, \mathbf{z})$ satisfies that every component of $\mathbf{w}, \mathbf{z}$ is a non-negative integer multiple of $\dfrac{1}{k}$, so it naturally follows that $\max(|\operatorname{supp}(\mathbf{w})|, |\operatorname{supp}(\mathbf{z})|) \le \kappa(\delta)$, i.e., both supports have size at most $\kappa(\delta)$. There are $O(n^{\kappa(\delta)})$ such strategies per player and hence $O(n^{2\kappa(\delta)})$ profiles. Since the lemma guarantees existence, it suffices to enumerate all profiles and check whether each one is a $(1/2+\delta)$-WSNE. When $\delta$ is a constant, the running time of the algorithm is polynomial in $n$.

\begin{remark}
  In Lemma~\ref{lem:llm_thm}, the sampling size $k = O\left(\dfrac{\log n}{\varepsilon^2}\right)$ depends on $n$, while here $\kappa(\delta)$ depends only on constant $\delta$. This is (probably) because in this case we only need to constrain the sampled support, whereas the LMM lemma needs to constrain all $n$ pure strategies.
\end{remark}
...
Fearnley and Savani extended this result to the $\varepsilon$-WSNE case: with $O\left(\frac{n \log n}{\varepsilon^4}\right)$ queries, one can output an $\varepsilon$-WSNE with success probability at least $\left( 1 - n^{-\frac{1}{8}} \right)\left( 1 - \frac{2}{n} \right)^2$ \cite[Corollary~5.5]{fearnleySavani16}.
...
\begin{lemma}[{\cite[Lemma~1]{12WSNE} (implied by the proof of Lemma \ref{lem:llm_thm} \cite[Theorem~1]{lmm03})}]
...
\begin{lemma}[{\cite[Lemma~2]{12WSNE}}]
  ...
  The proof is given in Appendix~\ref{app:sampled-wsne-proof}.
\end{lemma}

The randomized implementation and its failure probability are specified in Section~\ref{sec:source-corrections}.
...
Its correctness proof and precise guarantee are given in Appendix~\ref{app:case-two-low}.
...
Its correctness proof is given in Appendix~\ref{app:case-two-low-high}.
...
\]
The contradiction argument is given in Appendix~\ref{app:case-two-both-high}.
...
Surprisingly, we found a few minor errors in the original paper that do not affect its main conclusions.
...
\]
Corollary~5.5 of Fearnley and Savani guarantees success probability at least $p_n$ for one invocation of the zero-sum routine \cite[Corollary~5.5]{fearnleySavani16}, whereas Algorithm~2 invokes it \textbf{twice}. With independent random coins, both invocations therefore succeed with probability at least $p_n^2$. Moreover, Lemma~1 is existential, but a randomized implementation must draw the small-support profile and include the sampling event. Hoeffding bounds and a union bound over its constraints show that
\[
...
The source also contains minor local slips that do not affect its conclusions. In particular, the proof of Theorem~2 invokes Lemma~1 in Case~3(b), although the required $3\varepsilon$-WSNE conclusion is supplied by Lemma~2; the query-count paragraph repeats the same support where the symmetric support is intended; and the introduction gives an $\varepsilon^{-2}$ rather than $\varepsilon^{-4}$ dependence for the zero-sum WSNE routine.
