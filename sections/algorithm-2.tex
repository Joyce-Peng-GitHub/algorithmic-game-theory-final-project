...
\begin{enumerate}
  ...
  \item Draw $\mathbf{x}_s^*, \mathbf{y}_s^*$ with the sample size from Section~\ref{sec:source-corrections}, query all $(e_i, e_j)$ with $i \in \operatorname{supp}(\mathbf{x}_s^*)$, and construct the restricted game $(\mathcal{R}_{\mathbf{x}_s^*}, \mathcal{C}_{\mathbf{x}_s^*})$.
\end{enumerate}
