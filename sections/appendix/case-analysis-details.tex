...
\begin{itemize}
  \item Row player: by Lemma 2, $(\mathbf{x}_s^*, \mathbf{y}_s^*)$ is a $3\varepsilon$-WSNE, and $\mathbf{x}'$ is supported on $\supp(\mathbf{x}_s^*)$, which means that every pure strategy in the support of $\mathbf{x}'$ is a $3\varepsilon$-best response;
...
\textbf{Proof:} For any $i \in \supp(\mathbf{x}_s) \subseteq \supp(\mathbf{x})$, Lemma 1 and the $\varepsilon$-WSNE property give
\begin{align*}
