...
\[
  \begin{cases}
    \sum_{i \in \operatorname{supp}(\mathbf{x}^*)} \mathbf{x}'(i) \cdot \mathcal{C}_{ij} \le \frac12, & \forall j \in [n]                               \\[4pt]
...
Define the \textbf{restricted game} $(\mathcal{R}_{\mathbf{x}^*}, \mathcal{C}_{\mathbf{x}^*})$: the row player may only use actions in $\operatorname{supp}(\mathbf{x}^*)$, while the column player may use all $n$ actions.
